
import { useEffect, useState } from "react"
import { Button, Card, CardBody, CardTitle, Modal, ModalBody, ModalFooter, ModalHeader, Table } from "reactstrap";
import useDistMove from "../../hooks/useDistMove"
import FetchingModal from "../common/FetchingModal";
import PageComponent from "../common/PageComponent";
import { getListInfo } from "../../api/LicenseApi";

const initState = { //PageResponseDTO
    dtoList: [],
    pageNumList: [],
    pageRequestDTO: null,
    prev: false,
    next: false,
    totalCount: 0,
    prevPage: 0,
    nextPage: 0,
    totalPage: 0,
    current: 0
}



function LicenseInfoModal({ isOpen, callbackFn, resultLno }) {
    const { page, size, refresh, moveToInfoList, moveToRead } = useDistMove();
    const [serverInfoData, setserverInfoData] = useState({ ...initState }); //리스트 데이터 받기
    //const [fetching, setFetching] = useState(false); //진행모달



    useEffect(() => {
        // setFetching(true)
        getListInfo({ page, size }).then(data => {
            console.log(data);
            setserverInfoData(data);
            // setFetching(false);
        })
        //.catch(err=> exceptionHandle(err))
    }, [page, size, refresh])

    const selectLno = (lno) => {
        resultLno = lno;
    }


    return (
        <Modal isOpen={isOpen} toggle={callbackFn}>
            <ModalHeader toggle={callbackFn}>INFO LIST</ModalHeader>
            <ModalBody>
                <div>
                    <Card>
                        <CardTitle tag="h5" className="d-flex justify-content-between align-items-center border-bottom p-3 mb-0">라이선스 상품 리스트</CardTitle>
                        <CardBody>
                            <Table className="no-wrap mt-3 align-middle" responsive borderless>
                                <thead>
                                    <tr>
                                        <th>상품명</th>
                                        <th>버전</th>
                                        <th>사용처</th>
                                        <th>저작권자</th>
                                        <th>금액/단위</th>
                                        <th>선택</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {serverInfoData.dtoList.map((infoDto, index) => (
                                        <tr key={infoDto.lno} className="border-top" >
                                            <td>{infoDto.rightName}</td>
                                            <td>{infoDto.version}</td>
                                            <td>{infoDto.purpose} </td>
                                            <td>{infoDto.copyrightHolder}</td>
                                            <td>{infoDto.totalPrice}/{infoDto.priceUnit}</td>
                                            <td>{infoDto.expireDate}</td>
                                            <td><Button className="btn-sm" color="info" onClick={(e) => selectLno(infoDto.lno)}>선택</Button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </Table>
                        </CardBody>
                        <div className="mb-3">
                            <PageComponent serverInfoData={serverInfoData} movePage={moveToInfoList}></PageComponent> </div>
                    </Card>
                </div>
            </ModalBody>
            <ModalFooter>
                <Button color="secondary" onClick={callbackFn}>닫기</Button>
            </ModalFooter>
        </Modal>
    );
}

export default LicenseInfoModal;